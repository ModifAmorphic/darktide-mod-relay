...
void test_build_chunk_sets_both_path_globals_and_opens_entry(void) {
    ...
    /* Log-level global, unset (NULL) -> empty string. */
    ASSERT_NOTNULL(strstr(out, "RELAY_LOG_LEVEL = \"\""));
...
void test_build_chunk_null_mod_path_emits_empty_global(void) {
    ...
    int n = trampoline_build_chunk("Z:\\mod_loader", NULL, NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "MOD_LOADER_DIR = \"Z:\\\\mod_loader\""));
...
void test_build_chunk_null_version_emits_nil_without_skipping_loader(void) {
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", NULL, 0, 0, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
...
void test_build_chunk_empty_version_emits_nil(void) {
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "", 0, 0, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "MOD_RELAY_VERSION = nil"));
}
...
void test_build_chunk_empty_loader_dir_rejected(void) {
    char out[64];
    ASSERT_EQ(-1, trampoline_build_chunk("", "Z:\\mods", NULL, "Z:\\t.lua", "0.2.0", 0, 0, NULL, out, sizeof(out)));
}

void test_build_chunk_empty_entry_rejected(void) {
    char out[64];
    ASSERT_EQ(-1, trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL, "", "0.2.0", 0, 0, NULL, out, sizeof(out)));
}

void test_build_chunk_null_args(void) {
    char out[64];
    /* mod_loader_dir NULL -> rejected. */
    ASSERT_EQ(-1, trampoline_build_chunk(NULL, "Z:\\mods", NULL, "Z:\\t.lua", "0.2.0", 0, 0, NULL, out, sizeof(out)));
    /* entry_path NULL -> rejected. */
    ASSERT_EQ(-1, trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL, NULL, "0.2.0", 0, 0, NULL, out, sizeof(out)));
    /* out NULL -> rejected. */
    ASSERT_EQ(-1, trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL, "Z:\\t.lua", "0.2.0", 0, 0, NULL, NULL, sizeof(out)));
    /* zero cap -> rejected. */
    ASSERT_EQ(-1, trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL, "Z:\\t.lua", "0.2.0", 0, 0, NULL, out, 0));
...
void test_build_chunk_round_trips_long_paths(void) {
    ...
    char out[1024];
    int n = trampoline_build_chunk(loader, mods, NULL, entry, "0.2.0", 0, 0, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
...
void test_build_chunk_skip_splash_enabled_emits_one(void) {
    ...
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 1, 0, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
...
void test_build_chunk_mods_in_game_tree_enabled_emits_one(void) {
    ...
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 1, NULL, out, sizeof(out));
    ASSERT_TRUE(n > 0);
...
void test_build_chunk_log_level_set_baked_verbatim(void) {
    /* A set level value is baked VERBATIM — never canonicalized in C; each
     * consumer (the native filter, the loader's trace gate) applies its own
     * matching. Unknown-but-plausible values and case are preserved. */
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, "trace",
                                   out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "RELAY_LOG_LEVEL = \"trace\""));
    /* The other globals are unaffected by the level being set. */
    ASSERT_NOTNULL(strstr(out, "MOD_LOADER_DIR = \"Z:\\\\mod_loader\""));
    ASSERT_NOTNULL(strstr(out, "RELAY_MOD_PATH = \"Z:\\\\mods\""));
    ASSERT_NOTNULL(strstr(out, "io.open(\"Z:\\\\mod_loader\\\\t.lua\", \"r\")"));

    char out2[1024];
    int n2 = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                    "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, "TRACE",
                                    out2, sizeof(out2));
    ASSERT_TRUE(n2 > 0);
    ASSERT_NOTNULL(strstr(out2, "RELAY_LOG_LEVEL = \"TRACE\""));  /* case kept */

    char out3[1024];
    int n3 = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                    "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, "debug",
                                    out3, sizeof(out3));
    ASSERT_TRUE(n3 > 0);
    ASSERT_NOTNULL(strstr(out3, "RELAY_LOG_LEVEL = \"debug\""));  /* non-trace bakes too */
}

void test_build_chunk_log_level_quote_and_backslash_are_escaped(void) {
    /* Escapable bytes in the raw value must be Lua-escaped so the parser
     * yields the original bytes (same escape path as the roots). */
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, "tr\"a\\ce",
                                   out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "RELAY_LOG_LEVEL = \"tr\\\"a\\\\ce\""));
}

void test_build_chunk_null_and_empty_log_level_emit_empty_global(void) {
    /* NULL (env unset) and "" both bake the empty-string global — the unset
     * form the loader's trace gate treats as off. */
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, NULL,
                                   out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "RELAY_LOG_LEVEL = \"\""));
    ...
    char out2[1024];
    int n2 = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                    "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, "",
                                    out2, sizeof(out2));
    ASSERT_TRUE(n2 > 0);
    ASSERT_NOTNULL(strstr(out2, "RELAY_LOG_LEVEL = \"\""));
}

void test_build_chunk_overlong_log_level_fails_safely_to_empty_global(void) {
    /* A value over the 16-char transport ceiling (the shell's level-read
     * buffer; the longest level name is 5) fails safely to the unset form —
     * the chunk still builds and the entry still loads. */
    char level[33];
    memset(level, 'x', sizeof(level) - 1);
    level[sizeof(level) - 1] = '\0';
    char out[1024];
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL,
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 0, 0, level,
                                   out, sizeof(out));
    ASSERT_TRUE(n > 0);
    ASSERT_NOTNULL(strstr(out, "RELAY_LOG_LEVEL = \"\""));
    ASSERT_NOTNULL(strstr(out, "return \"OK\""));
...
void test_build_chunk_globals_in_template_order(void) {
    /* All seven baked globals appear in the template's fixed order, before the
    ...
    int n = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", "Z:\\tools\\mgr.exe",
                                   "Z:\\mod_loader\\t.lua", "0.2.0", 1, 1, "trace", out, sizeof(out));
    ASSERT_TRUE(n > 0);
    const char *names[] = {
        "MOD_LOADER_DIR = ", "RELAY_MOD_PATH = ", "RELAY_MOD_MANAGER = ",
        "MOD_RELAY_VERSION = ", "RELAY_SKIP_SPLASH = ",
        "RELAY_MODS_IN_GAME_TREE = ", "RELAY_LOG_LEVEL = ",
        "local f, err = io.open(",
    };
    const char *prev = out;
...
void test_join_feeds_build_chunk(void) {
    ...
    char chunk[1024];
    int cn = trampoline_build_chunk("Z:\\mod_loader", "Z:\\mods", NULL, path, "0.2.0", 0, 0, NULL, chunk, sizeof(chunk));
    ASSERT_TRUE(cn > 0);
...
int main(void) {
    ...
    test_register("build_chunk_log_level_set_baked_verbatim",
                  test_build_chunk_log_level_set_baked_verbatim);
    test_register("build_chunk_log_level_quote_and_backslash_are_escaped",
                  test_build_chunk_log_level_quote_and_backslash_are_escaped);
    test_register("build_chunk_null_and_empty_log_level_emit_empty_global",
                  test_build_chunk_null_and_empty_log_level_emit_empty_global);
    test_register("build_chunk_overlong_log_level_fails_safely_to_empty_global",
                  test_build_chunk_overlong_log_level_fails_safely_to_empty_global);
